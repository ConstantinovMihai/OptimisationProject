...
def VRP_Problem (depots, customers, trucks, nodes, costs, alpha, gamma, distance):
    ...
    for epsilon in range(23500,30000,500):
        ...
        second_obj = model.addVar(vtype = "C", name = f"second_objective value")
        ...
        second_obj_value_constraint = model.addConstr(quicksum(node_i.cost*y[node_i.id] for node_i in depots)+
                            quicksum(quicksum(trucks[0].F*a[node_i.id,node_j.id] for node_i in depots) for node_j in customers)+
                            quicksum(quicksum(costs[node_i.id,node_j.id]*x[node_i.id,node_j.id] for node_i in nodes) for node_j in nodes)+
                            quicksum(quicksum(costs[node_i.id,node_j.id]*a[node_i.id,node_j.id] for node_i in depots) for node_j in customers),'=',second_obj,name=f'second objectvie value')
...
if __name__ == '__main__':
    ...
    # I, J
    depots, customers, trucks = generateInput(5,50)
